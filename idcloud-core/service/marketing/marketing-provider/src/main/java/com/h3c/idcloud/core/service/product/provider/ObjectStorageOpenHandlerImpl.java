 ...
 *
 */
//@Service("objectStorageOpenHandlerImpl")
@Service(version = "1.0.0",group = "objectStorageOpenHandlerImpl")
@Component("objectStorageOpenHandlerImpl")
public class ObjectStorageOpenHandlerImpl implements ResourceRequestHanlder {

	private static final Logger logger = LoggerFactory.getLogger(ObjectStorageOpenHandlerImpl.class);
...
//		record.setAccount(AuthUtil.getAuthUser().getAccount());--------wsl
		...
		try {
			...
			if(orderId != null) {
				...
				criteria.put("serviceSid", WebConstants.ServiceSid.SERVICE_OBJ_STORAGE);
				...
				for (ServiceInstance serviceInstance : serviceInstances) {
					...
					ResObjStoInst resObjStoInst = new ResObjStoInst();
					resObjStoInst.setMgtObjSid(mgtObjSid);
					resObjStoInst.setAccount(serviceInstance.getOwnerId());

					logger.info("createObjStorage input params:" + JsonUtil.toJson(resObjStoInst));
					ResInstResult resInstResult = this.resObjStorageInstService.createObjStorage(resObjStoInst);
//					ResInstResult resInstResult = this.mockResVmService.createObjStorage(resObjStoInst);
					logger.info("createObjStorage return result:" + JsonUtil.toJson(resInstResult));
...
//			log.debug(record);
	...
	public void saveInstanceRes(Order order, ServiceInstance serviceInstance, ResInstResult resInstResult, Long mgtObjSid, Long instanceSid) {
		if(ResInstResult.SUCCESS == resInstResult.getStatus()) {
			...
			//保存实例和资源关联关系
			ResObjStorageInst resObjStorage = (ResObjStorageInst)resInstResult.getData();
			ServiceInstRes serviceInstRes = new ServiceInstRes();
			serviceInstRes.setInstanceSid(serviceInstance.getInstanceSid());
			serviceInstRes.setResId(resObjStorage.getResInstSid());
...
//			instanceSpecService.modifyInstanceSpecToChanging(serviceInstance.getInstanceSid());
			...
			//更新实例变更日志
			changeLogService.endChangeServiceInstance(serviceInstance, WebConstants.instanceChangeType.CREATE);
	...
	public void saveOpenErrorTicket(String orderId, Long mgtObjSid, Long instanceSid, String errorMessage, Boolean isResSend) {
		try {
			...
			criteria.put("processType", WebConstants.TicketProcessType.OBJECT_STORAGE_OPEN);
			...
			if (!ticketList.isEmpty()){
				...
				if(!ticketRecords.isEmpty()) {
					...
					ticketRecord.setOperateContent("对象存储开通工单自动处理失败");
			...
			ticket.setProcessType(WebConstants.TicketProcessType.OBJECT_STORAGE_OPEN);
	...
	public void operate(ResInstResult result) {
		try {
			logger.info("createObjStorage callback result:" + JsonUtil.toJson(result));
...
//					serviceInstance.setStatus(WebConstants.ServiceInstanceStatus.OPENED);
	...
	public void handlerData(ServiceInstance instance, String resVdSid) {
		...
		if(instanceService.checkOrderSuccess(orderId, WebConstants.ServiceSid.SERVICE_OBJ_STORAGE)) {
			//更新订单状态为已完成
			Criteria criteria = new Criteria();
			criteria.put("orderId", orderId);
			List<Order> orders = orderService.selectByParams(criteria);
		...
		//更新工单信息
		ticketService.modifyTicketStatus(instanceSid, WebConstants.ProcessType.OBJECT_STORAGE_OPEN);
		//发送邮件通知
