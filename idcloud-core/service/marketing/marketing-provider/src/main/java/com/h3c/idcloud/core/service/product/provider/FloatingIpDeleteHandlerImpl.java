...
import com.h3c.idcloud.core.pojo.instance.ResFloatingIpInst;
 ...
 *
 */
@Service(version = "1.0.0",group = "floatingIpDeleteHandlerImpl")
@Component("floatingIpDeleteHandlerImpl")
public class FloatingIpDeleteHandlerImpl implements ResourceRequestHanlder {

	private static final Logger logger = LoggerFactory
			.getLogger(FloatingIpDeleteHandlerImpl.class);
...
//		record.setAccount(AuthUtil.getAuthUser().getAccount()); --------wsl
		...
		try {
			...
			if(serviceInstReses.size() > 0) {
				floatingIpId = serviceInstReses.get(0).getResId();
			}
			...
			logger.info("deleteFloatingIp input params: floatingIpId=" + floatingIpId);
			ResFloatingIpInst resFloatingIPInst = new ResFloatingIpInst();
			resFloatingIPInst.setResSid(floatingIpId);
			resFloatingIPInst.setMgtObjSid(serviceInstance.getMgtObjSid());
			// TODO with new interface by MCH
			ResInstResult resInstResult = floatingIpService.deleteFloatingIP(new ResCommonInst());
			logger.info("deleteFloatingIp result params:" + JsonUtil.toJson(resInstResult));
			modifyChangeFloatingIpData(resInstResult, serviceInstance);
...
//			log.debug(record);
	...
	@Transactional
	public void modifyChangeFloatingIpData(ResInstResult resInstResult, ServiceInstance serviceInstance) {
...
//							vmService.unsubscribeVmCallBack(orderId);
						}
					}
				} else {
					//发送退订通知邮件
