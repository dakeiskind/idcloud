...
/**
 * The OpenStack Compute API is extensible. Extensions serve two purposes: They allow the introduction of new features
 * in the API without requiring a version change and they allow the introduction of vendor specific niche
 * functionality.
 *
...
//        this.namespace = checkNotNull(namespace, "namespace");
        if("router".equals(alias)) {
            this.namespace = URI.create(ExtensionNamespaces.L3_ROUTER);
        }else {
            this.namespace = namespace == null ? URI.create(""):namespace;
        }
