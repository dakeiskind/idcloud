...
@Service
public class VmwareVmNicAdd extends ActionVmware {

    private static Logger log = LoggerFactory.getLogger(VmwareVmNicAdd.class);

    @Override
    protected CommonAdapterResult execute(Base base) throws VmwareException,
            AdapterUnavailableException {

        VmNicAdd vmNicAdd = (VmNicAdd) base;

        CommonAdapterResult result = new CommonAdapterResult();

        String json = helper.getParam(vmNicAdd.getProviderType(), vmNicAdd);

        log.info("vm add nic post data : " + json);
        ...
        try {
            Response resp = client.path("/vmnic/add")
            ...
            log.info("vmware adapter is not available when adding nic");
