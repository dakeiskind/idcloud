...
@Service
public class KvmBlockCreateSnapshot extends ActionKvm {
    ...
    protected CommonAdapterResult execute(Base base) throws KvmException,
        ...
        BlockSnapshotCreate blockSnapshotCreate = (BlockSnapshotCreate) base;


        String json = helper.getParam(blockSnapshotCreate.getProviderType(), blockSnapshotCreate);
        ...
        try {
            Response resp = client.path("/" + blockSnapshotCreate.getTenantId() + "/volumes/" + blockSnapshotCreate.getVolumeId() + "/volume_snapshots").type(MediaType.APPLICATION_JSON).post(json);

            if (resp.getStatus() == 200) {
                respMap = resp.readEntity(Map.class);
                if (respMap.containsKey("volume_snapshot")) {
                    block.setBlockSnapshotCreateResult(BaseUtil.castObject(respMap.get("volume_snapshot"), BlockSnapshotCreateResult.class));
                    block.getBlockSnapshotCreateResult().setSuccess(true);
                }
            ...
            log.info("kvm adapter is not available when creating BlockSnapshot");
