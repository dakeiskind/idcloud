...
@Service
public class VmwareVmRename extends ActionVmware {

    private static Logger log = LoggerFactory.getLogger(VmwareVmRename.class);

    @Override
    protected CommonAdapterResult execute(Base base) throws VmwareException, AdapterUnavailableException {

        VmRename vmRename = (VmRename) base;

        CommonAdapterResult result = new CommonAdapterResult();

        String json = helper.getParam(vmRename.getProviderType(), vmRename);

        log.info("vm rename post data : " + json);
        ...
        try {
            Response resp = client.path("/vm/renameVm/").type(MediaType.APPLICATION_JSON).post(json);
