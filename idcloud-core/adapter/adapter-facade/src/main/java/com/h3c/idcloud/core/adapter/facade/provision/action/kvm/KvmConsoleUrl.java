...
@Service
public class KvmConsoleUrl extends ActionKvm {

    private static Logger log = LoggerFactory.getLogger(KvmConsoleUrl.class);
    ...
    protected CommonAdapterResult execute(Base base) throws KvmException,
        ...
        VmConsole vmConsole = new VmConsole();
        String json = helper.getParam(vmVncConsole.getProviderType(), vmVncConsole);

        log.info("vm getVncConssole post data :" + json);
        ...
        try {
            Response resp = client
                    .path("/" + vmVncConsole.getTenantId() + "/servers/"
                            + vmVncConsole.getId() + "/action")
                    .type(MediaType.APPLICATION_JSON).post(json);

            if (resp.getStatus() == 200) {
                respMap = resp.readEntity(Map.class);
                if (respMap.containsKey("console")) {
                    Map serverMap = (Map) respMap.get("console");
                    vmConsole.setUrl((String) serverMap.get("url"));
                }
                vmConsole.setSuccess(true);
            } else {
            ...
            log.info("kvm adapter is not available when getConsoleUrl vm");
