...
@Service
public class KvmUserDelete extends ActionKvm {

    private static Logger log = LoggerFactory.getLogger(KvmUserDelete.class);

    @Override
    protected CommonAdapterResult execute(Base base) throws KvmException, AdapterUnavailableException {

        UserDelete userDelete = (UserDelete) base;

        Boolean flag = false;

        CommonAdapterResult result = new CommonAdapterResult();

        log.info("user deletting...");
        ...
        try {
            Response resp = client.path("/users/" + userDelete.getUserId()).type(MediaType.APPLICATION_JSON).delete();
            ...
            log.info("kvm adapter is not available when deleting user");
