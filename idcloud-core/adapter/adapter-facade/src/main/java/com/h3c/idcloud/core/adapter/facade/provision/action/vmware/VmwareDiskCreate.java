...
@Service
public class VmwareDiskCreate extends ActionVmware {

    private static Logger log = LoggerFactory.getLogger(VmwareDiskCreate.class);
    ...
    protected CommonAdapterResult execute(Base base) throws VmwareException,
        ...
        String json = helper.getParam(diskCreate.getProviderType(),
                diskCreate);
        ...
        try {
            Response resp = client.path("/disk/create/")
                    .type(MediaType.APPLICATION_JSON).post(json);

            this.checkResponseStatus(resp);

            if (resp.getStatus() == 200) {
                disk = resp.readEntity(Disk.class);
                disk.setSuccess(true);
            } else {
            ...
            log.info("vmware adapter is not available when creating disk");
