...
@Service
public class VmwareEnvTest extends ActionVmware {

    @Override
    protected CommonAdapterResult execute(Base base) throws VmwareException,
            AdapterUnavailableException {
        EnvConnectionTest envConnectionTest = (EnvConnectionTest) base;
        String json = helper.getParam(envConnectionTest.getProviderType(), envConnectionTest);

        log.info("connectting env test post data : " + json);
        ...
        try {
            Response resp = client.path("/vCenter/testVcenter").type(MediaType.APPLICATION_JSON).post(json);
