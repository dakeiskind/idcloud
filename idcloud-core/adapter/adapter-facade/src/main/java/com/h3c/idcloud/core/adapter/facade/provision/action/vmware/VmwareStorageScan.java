...
@Service
public class VmwareStorageScan extends ActionVmware {

    private static Logger log = LoggerFactory.getLogger(VmwareStorageScan.class);

    @Override
    protected CommonAdapterResult execute(Base base) throws VmwareException, AdapterUnavailableException {

        StorageScan storageScan = (StorageScan) base;

        String json = helper.getParam(storageScan.getProviderType(), storageScan);

        log.info("storage scan post data : " + json);
        ...
        try {
            Response resp = client.path("/cluster/getStorages").type(MediaType.APPLICATION_JSON).post(json);

            this.checkResponseStatus(resp);

            if (resp.getStatus() == 200) {
                List<DataStore> listDataStore = resp.readEntity(new GenericType<List<DataStore>>() {
                });
                dataStores.setListdata(listDataStore);
            } else {
            ...
            log.info("vmware adapter is not available when scanning datastores");
