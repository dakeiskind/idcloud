...
@Service
public class KvmImageDelete extends ActionKvm {

    @Override
    protected CommonAdapterResult execute(Base base) throws KvmException,
            AdapterUnavailableException {
        ImageDelete vmImageDelete = (ImageDelete) base;
        ...
        try {
            Response resp = client.path("/" + vmImageDelete.getTenantId() + "/images/" + vmImageDelete.getImageId()).type(MediaType.APPLICATION_JSON).delete();
            ...
            log.info("kvm adapter is not available when image deleting");
