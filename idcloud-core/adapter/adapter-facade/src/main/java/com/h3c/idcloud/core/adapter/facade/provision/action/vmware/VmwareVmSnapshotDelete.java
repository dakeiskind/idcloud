...
@Service
public class VmwareVmSnapshotDelete extends ActionVmware {

    private static Logger log = LoggerFactory.getLogger(VmwareVmSnapshotDelete.class);
    ...
    protected CommonAdapterResult execute(Base base) throws VmwareException,
        ...
        String json = helper.getParam(vmSnapshotDelete.getProviderType(),
                vmSnapshotDelete);

        log.info("vm snapshot delete post data : " + json);
        ...
        try {
            Response resp = client.path("/vm/snapshot/delete")
            ...
            log.info("vmware adapter is not available when deleting snapshot");
