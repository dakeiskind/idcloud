...
@Service
public class VmwareVmReconfig extends ActionVmware {

    private static Logger log = LoggerFactory.getLogger(VmwareVmReconfig.class);

    @Override
    protected CommonAdapterResult execute(Base base) throws VmwareException,
            AdapterUnavailableException {

        VmReconfig vmReconfig = (VmReconfig) base;

        CommonAdapterResult result = new CommonAdapterResult();

        String json = helper.getParam(vmReconfig.getProviderType(),
                vmReconfig);
        ...
        try {
            Response resp = client.path("/vm/reconfig")
            ...
            log.info("vmware adapter is not available when reconfiging vm");
