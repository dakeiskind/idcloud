...
@Service
public class VmwareDiskDelete extends ActionVmware {

    private static Logger log = LoggerFactory.getLogger(VmwareDiskDelete.class);
    ...
    protected CommonAdapterResult execute(Base base) throws VmwareException,
        ...
        String json = helper.getParam(diskDelete.getProviderType(),
                diskDelete);

        log.info("disk delete post data : " + json);
        ...
        try {
            Response resp = client.path("/disk/delete/")
            ...
            log.info("vmware adapter is not available when deleting vm");
