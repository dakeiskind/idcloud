...
@Service
public class KvmVmResize extends ActionKvm {

    private static Logger log = LoggerFactory.getLogger(KvmVmResize.class);

    @Override
    protected CommonAdapterResult execute(Base base) throws KvmException,
            AdapterUnavailableException {
        VmResize vmResize = (VmResize) base;

        Server server = new Server();
        String json = helper.getParam(vmResize.getProviderType(), vmResize);
        ...
        try {
            Response resp = client.path("/" + vmResize.getTenantId() + "/servers/" + vmResize.getId() + "/action").type(MediaType.APPLICATION_JSON).post(json);
            ...
            log.info("kvm adapter is not available when resizing vm");
