package com.h3c.idcloud.core.adapter.facade.provision.action.monitor.ceilometer;
...
public abstract class ActionCeilometer {

    protected static Logger log = LoggerFactory.getLogger(ActionCeilometer.class);
    @Autowired
    ...
    public ActionCeilometer() {
        JacksonJaxbJsonProvider jsonProvider = new JacksonJaxbJsonProvider();
        providers.add(jsonProvider);

        URL adapter_ = super.getClass().getResource(
                "/adapter.monitor.properties");

        try {
            adapter.load(adapter_.openStream());
            setAdapterUrl(adapter.getProperty("ceilometer"));
        } catch (IOException e) {
