...
@Service
public class VmwareHostScanAlone extends ActionVmware {

    private static Logger log = LoggerFactory.getLogger(VmwareHostScanAlone.class);
    ...
    protected CommonAdapterResult execute(Base base) throws VmwareException, AdapterUnavailableException {
        ...
        String json = helper.getParam(hostScanAlone.getProviderType(), hostScanAlone);
        log.info("host scan alone post data : " + json);
        try {
            Response resp = client.path("/host/asyncHostInfo/" + hostScanAlone.getName())
                    .type(MediaType.APPLICATION_JSON).post(json);
            this.checkResponseStatus(resp);
            if (resp.getStatus() == 200) {
                host = resp.readEntity(Host.class);
            ...
            log.info("vmware adapter is not available when scanning host alone");
