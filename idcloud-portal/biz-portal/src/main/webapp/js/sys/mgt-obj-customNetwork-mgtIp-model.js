var customNetworkViewIpGrid = function(){
	var me = this;
    ...
    this.searchIpConfigInfo = function(resNetworkSid){
    	 ...
    	 Core.AjaxRequest({
 			...
 			callback : function (data) {
 			    ...
 			    $("#customnNetworkIpConfigMgtdatagrid").jqxGrid({source: dataAdapter});
 			}
 		 });
    };
 ...
 // 判断地址是否重复
		...
		$('#addPniNetworkOwnIpForm').on('validationSuccess', function (event) {
	    		 ...
	    		 Core.AjaxRequest({
	 				...
	 				callback : function (data) {
	 					...
	 					// 刷新基本信息
	 					var  pniIp = new customNetworkViewIpGrid();
    ...
    this.initInput = function(){
    	...
    	$("#search-custom-network-IP-address").jqxInput({placeHolder: "", height: 22, width: 100, minLength: 1,theme:currentTheme});
    	ipconfig.getCommonCode("search-custom-network-IP-usage-status","RES_IP_STATUS",true);
        $("#search-custom-network-ip-button").jqxButton({ width: '50px',height:'25px',theme:currentTheme});
        $("#customNetworkIpCancel").jqxButton({ width: '50px',height:'25px',theme:currentTheme});
    ...
    this.initIpDatagrid = function(){
          $("#customnNetworkIpConfigMgtdatagrid").jqxGrid({
              width: "99.5%",
			  theme:currentTheme,
              columnsresize: true,
              pageable: true, 
          ...
          // 控制按钮是否可用
    	  $("#customnNetworkIpConfigMgtdatagrid").on('rowselect', function (event) {
    		  var args = event.args; 
    		  var index = args.rowindex;
    		  var data = $('#customnNetworkIpConfigMgtdatagrid').jqxGrid('getrowdata', index);
    ...
    this.initPopWindow = function(){
    	$("#viewCustomNetworkOwnIpGrid").jqxWindow({
		      width: 750, 
		      height:465,
		      theme:currentTheme,
		      resizable: false,  
		      isModal: true, 
		      autoOpen: false, 
		      cancelButton: $("#customNetworkIpCancel"), 
		      modalOpacity: 0.3
		  });
...
function searchNetworkIpConfigMgt(){
	...
	networkIp.searchObj.ipAddressLike = $("#search-custom-network-IP-address").val();
	networkIp.searchObj.ipType = $("#search-custom-network-IP-type").val()=="全部"?"":$("#search-custom-network-IP-type").val();
	networkIp.searchObj.usageStatus = $("#search-custom-network-IP-usage-status").val()=="全部"?"":$("#search-custom-network-IP-usage-status").val();
