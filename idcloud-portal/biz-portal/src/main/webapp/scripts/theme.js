function getDemoTheme() {
    ...
    var url = ctx + "/jqwidgets/styles/jqx." + theme + '.css';
        ...
        if (window.location.toString().indexOf('file://') >= 0) {
            ...
            if (addMessage) {
                $(document).ready(function () {
                    setTimeout(function () {
                        $(document.body).prepend($('<div style="font-size: 12px; font-family: 微软雅黑, Microsoft Yahei, Verdana;">Note: To run a sample that includes data binding, you must open it via "http://..." protocol since Ajax makes http requests.</div><br/>'));
                    }
                    , 50);
                });
            }
