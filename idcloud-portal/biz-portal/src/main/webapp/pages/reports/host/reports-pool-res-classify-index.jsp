...
<body>
   <div style="width:100%;height:30px;padding:5px 0px 5px 0px;">
           <table border="0" style="margin-left:10px" width="1050px" height="100%" cellspacing="0" cellpadding="2">
            	<tr>
            		<td align="left" width="80px" nowrap="nowrap">资源池类型：</td>
           			<td width="80px"><div id="search-pool-res-type" ></div></td>
   ...
   <div style="width:1050px;height:93%;margin-left:10px;">
		<iframe id="poolResClassifyCountIframe" src="" width="100%" height="100%" frameborder="no" border="1" style="border:1px solid #E5E5E5" marginwidth="0" marginheight="0"></iframe>
	</div>
</body>
<script type="text/javascript">
	$(function(){
		...
		var item = $("#search-pool-res-type").jqxDropDownList('getSelectedItem');
		var objSearch = {
			timeType : 'year',
			poolType:item.value,
			poolText:item.label,
		...
		$("#poolResClassifyCountIframe").attr("src",ws_url + "/rest/reports/host/jasper/poolResClassifyCount/"+json).load(function() { 
			if($("#poolResClassifyCountIframe").contents().find("#_blank").length > 0){
	...
	function initInput(){
		...
		var source ={
         ...
         $("#search-pool-res-type").jqxDropDownList({
             selectedIndex: 0, 
             source: dataAdapter, 
             displayMember: "name", 
             valueMember: "value", 
	...
	function downloadFile(obj,type){
		...
		if(!isok){
			...
			if("year" == timeType){
				...
				var json = JSON.stringify(objSearch);
				window.location = ws_url + "/rest/reports/host/export/jasper/poolResClassifyCount/"+json; 
				...
				var json = JSON.stringify(objSearch);
				window.location = ws_url + "/rest/reports/host/export/jasper/poolClassifyCount/"+json; 
