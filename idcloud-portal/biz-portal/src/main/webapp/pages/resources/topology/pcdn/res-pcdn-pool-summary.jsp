...
<!-- 	  		         <font style="position:absolute;right:5px"><i class="icons-blue icon-help"></i></font> -->
	        ...
	        <div>
	        	<table border="0" width="97%" style="margin-left:3%" cellspacing="5" cellpadding="0">
	        		<tr>
	        			<td align="left" height="25px"><span style="cursor:pointer;color:#3B9AFF" onclick="popAddCDNPoolWindow()"><i class="icon-list-add" ></i>&nbsp;新增CDN</span></td>
	        		</tr>
	        	</table>
	        </div>
  		</div>
  	...
  	<div style="width:98%;height:370px;margin-left:1%;">
  		<div class="customPanel" style="width:58%;height:350px;margin-left:41.7%;">
  		    <div class="title">&nbsp;&nbsp;资源统计
  		    	 <font style="position:absolute;right:5px;cursor:pointer" onclick="initPoolPCDNSummary()"><i class="icons-blue icon-arrows-ccw"></i></font>
  		    </div> 
        <div>
        	<!-- 对象存储资源总数 -->
        	<div id='cdnCountChart' style="width:50%;height:320px;margin:0 auto;border:0px"></div>
        	<!-- 对象存储资源使用情况 -->
        	<!-- <div id='cdnUsedChart' style="width:50%;float:left; height:320px;margin:0 auto;border:0px"></div>	 -->
        </div>
  		</div>
  	</div>
  </div>
...
<script type="text/javascript">
	...
	function initPoolPCDNSummary(){
		var  cdnPool= new poolPcdnDatagridModel();
		var osData = cdnPool.statisticsCDNInPool(resTopologySid);
		//var osUseData = cdnPool.statisticsCDNUseInPool(resTopologySid);
		// 初始化chart图
	 ...
	 // 初始化虚拟机图表
 	 function initOSCountChart(data){
         ...
         // setup the chart
         $('#cdnCountChart').jqxChart(settings);
	 }
